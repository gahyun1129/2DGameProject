...
class Pitcher:
    ...
    def __init__(self, x, y, frame, frame_number, action, team_color, name, strike_out, four_balls, ERA, pitching):
        # 위치, 현재 프레임, 현재 action, 프레임의 길이, 팀 색 결정
        self.x, self.y = x, y
        self.frame, self.frame_number, self.action = frame, frame_number, (action + team_color)
